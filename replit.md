...
- After pulling schema changes, run `pnpm --filter @workspace/db run push` again — this round added the `whisper_groups` and `whisper_group_members` tables plus `groupSendId`/`whisperGroupId` on `whisps`, on top of the prior round's `circles`, `circle_members`, `push_subscriptions` tables and `role`/`banned`/`country`/`region`/`city`/`lastSeenAt` on `users`, `videoTranscript` on `whisps`, and `whisp_categories`
