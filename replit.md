...
- After pulling schema changes, run `pnpm --filter @workspace/db run push` again — this round added the `circles`, `circle_members`, and `push_subscriptions` tables, `role`/`banned`/`country`/`region`/`city`/`lastSeenAt` on `users`, `videoTranscript` on `whisps`, and the new `whisp_categories` table
