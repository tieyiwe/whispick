...
describe("GET /wb/:handle", () => {
  ...
  it("serves a real Open Graph card to link-preview crawlers", async () => {
    ...
    const res = await request(app).get(`/wb/${handle}`).set("User-Agent", "WhatsApp/2.23.20 A");
  ...
  it("redirects (never unfurls) an unknown handle or a disabled box, even to a crawler", async () => {
    const unknown = await request(app).get(`/wb/${randomUUID()}`).set("User-Agent", "WhatsApp/2.23.20 A");
    ...
    const disabled = await request(app).get(`/wb/${handle}`).set("User-Agent", "WhatsApp/2.23.20 A");
