...
// once — "prefer_not_to_say" is a real answer that satisfies the gender/
// ageRange half just like any other option; there's no such option for
// preferredLanguage (see users.preferredLanguage's comment for why), so
// that half is only satisfied by a real, supported language code. This only
// checks for "never asked," not for any particular value.
export function needsDemographics(user: { gender: string | null; ageRange: string | null; preferredLanguage: string | null }): boolean {
  return !user.gender || !user.ageRange || !user.preferredLanguage || !isSupportedLanguage(user.preferredLanguage);
}
