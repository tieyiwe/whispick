...
export function PinToTaskbarTip() {
  ...
  const instructions = mac
    ? t("pinToTaskbarTip.instructionsMac")
    : t("pinToTaskbarTip.instructionsWindows");

  return (
    <div
      className="fixed inset-x-3 z-[60] bottom-4 md:left-auto md:right-4 md:w-96"
      role="status"
      aria-label={t("pinToTaskbarTip.ariaLabel")}
    ...
    >
      <div className="rounded-2xl border border-primary/30 bg-card/95 p-4 shadow-[0_8px_40px_rgba(0,0,0,0.45)] backdrop-blur flex items-start gap-3">
        <div className="min-w-0 flex-1">
          <p className="font-serif text-sm font-semibold text-foreground">{t("pinToTaskbarTip.title")}</p>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {t("pinToTaskbarTip.description", { instructions })}
          </p>
        </div>
        <button
          type="button"
          onClick={() => setVisible(false)}
          aria-label={t("pinToTaskbarTip.dismiss")}
