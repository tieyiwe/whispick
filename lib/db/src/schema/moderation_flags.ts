...
// A record of whisps (or, since Text Whisps shipped, text_whisps, and now
// circle_comments — see contentType below) an automated pass flagged as
// possibly containing sexual/explicit content (see lib/moderation.ts), for
// admin review — not an automated takedown or ban. userId is the sender,
// denormalized from the source row so "how many times has this person been
// flagged" is a plain count query, no join required. dismissed marks a
// false positive an admin has already looked at and cleared;
// reviewedAt/reviewedByAdminId record that review happened at all
...
export const moderationFlagsTable = pgTable("moderation_flags", {
  ...
  // Exactly one of whispId/textWhispId/circleCommentId is set, per
  // contentType below — whispId stayed NOT NULL for years before Text
  // Whisps existed, so it's relaxed to nullable here rather than backfilled
  // with a sentinel.
  ...
  contentType: text("content_type").notNull().default("whisp"), // 'whisp' | 'text_whisp' | 'circle_comment'
  // Nullable ONLY for contentType='circle_comment': a comment can come from
  // a fully anonymous, no-account visitor with no userId to attribute it
  // to. maybeWarnUser (lib/moderation.ts) is skipped when null — there's no
